# tests/ch8/test_belief_driven_planning.py
...
def test_planner_receives_beliefs():
    store = InMemoryStore()
    ...
    state = State(task_id="t1", user_input="research TSLA")
    orch.run(state)

    assert "beliefs" in state.plan.meta
    assert isinstance(state.plan.meta["beliefs"], list)
